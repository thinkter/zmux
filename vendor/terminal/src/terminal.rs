...
// zmux patch: halved from upstream's 10_000. zmux keeps every workspace's
// terminals alive, so scrollback dominates resident memory; users can still
// raise `terminal.max_scroll_history_lines` up to MAX_SCROLL_HISTORY_LINES.
