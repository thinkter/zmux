use crate::{git_status_icon, repository_scope};
...
impl RepositorySelector {
    pub fn new(
        ...
        let mut repository_entries = scope.repositories(&project_handle, cx);
        repository_entries.sort_by_key(|repo| scope.display_name(repo, cx).to_lowercase());
        ...
        let widest_item_ix = repository_entries.iter().position_max_by(|a, b| {
            scope
                .display_name(a, cx)
                .len()
                .cmp(&scope.display_name(b, cx).len())
        });
...
impl PickerDelegate for RepositorySelectorDelegate {
    ...
    fn update_matches(
        ...
        let repo_names: Vec<(Entity<Repository>, String)> = all_repositories
            ...
            .map(|repo| {
                (
                    repo.clone(),
                    repository_scope(cx).display_name(repo, cx).to_lowercase(),
                )
            })
            .collect();
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            this.update_in(cx, |this, window, cx| {
                ...
                sorted_repositories
                    .sort_by_key(|repo| repository_scope(cx).display_name(repo, cx).to_lowercase());
    ...
    fn confirm(&mut self, _secondary: bool, window: &mut Window, cx: &mut Context<Picker<Self>>) {
        ...
        repository_scope(cx).select(&self.project, selected_repo.clone(), cx);
    ...
    fn render_match(
        ...
        let display_name = repository_scope(cx).display_name(repo_info, cx);
