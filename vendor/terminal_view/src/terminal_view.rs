...
///
/// The hook runs before direct pane clones and persisted terminal restores are
/// mounted, allowing embedders to provision per-process environment or
/// capabilities without replacing a live terminal after the fact.
pub type TerminalCreationHandler = Arc<
    dyn Fn(
        WeakEntity<Project>,
        Option<PathBuf>,
        &mut App,
...
struct RegisteredTerminalCreationHandler(TerminalCreationHandler);

impl Global for RegisteredTerminalCreationHandler {}
...
impl Item for TerminalView {
    ...
    fn clone_on_split(
        ...
        let Ok(cwd) = self.project.update(cx, |project, cx| {
            project
        ...
        let handler = cx
            .try_global::<RegisteredTerminalCreationHandler>()
            .map(|handler| Arc::clone(&handler.0));
        let terminal = if let Some(handler) = handler {
            handler(self.project.clone(), cwd, cx)
        } else {
...
impl SerializableItem for TerminalView {
    ...
    fn deserialize(
        ...
        window.spawn(cx, async move |cx| {
            ...
            let handler = cx
                .update(|_, cx| {
                    cx.try_global::<RegisteredTerminalCreationHandler>()
                        .map(|handler| Arc::clone(&handler.0))
                })
                .ok()
                .flatten();
            let terminal = if let Some(handler) = handler {
                cx.update(|_, cx| handler(project.downgrade(), cwd, cx))?
                    .await?
            } else {
                project
