...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    fn advance_osc_string<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        match byte {
            ...
            0x3B => {
                ...
                // Notification payload text may contain arbitrary semicolons.
                // Once the fixed parameter table is full, preserve the entire
                // remaining tail in its last entry so the ANSI bridge can
                // reconstruct OSC 9/99/777 without truncation.
                if self.osc_num_params == MAX_OSC_PARAMS && self.is_zmux_notification_osc() {
                    self.action_osc_put(b';');
                }
            },
    ...
    fn is_zmux_notification_osc(&self) -> bool {
        if self.osc_num_params == 0 {
            return false;
        }

        let (start, end) = self.osc_params[0];
        matches!(&self.osc_raw[start..end], b"9" | b"99" | b"777")
    }

    fn osc_end<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        self.action_osc_put_param();
        if self.osc_num_params == MAX_OSC_PARAMS && self.is_zmux_notification_osc() {
            self.osc_params[MAX_OSC_PARAMS - 1].1 = self.osc_raw.len();
        }
...
mod tests {
    ...
    #[test]
    fn osc_param_references_are_valid_for_each_dispatch_in_a_batch() {
        const FIRST: &[&[u8]] = &[b"99", b"i=one:p=body", b"alpha", b"beta"];
        const SECOND: &[&[u8]] = &[b"9", b"second"];
        const THIRD: &[&[u8]] = &[b"777", b"notify", b"Third", b"gamma", b"delta"];
        ...
        impl Perform for ReferenceDispatcher {
            fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
                let (expected_params, expected_bell) = match self.osc_index {
                    0 => (FIRST, false),
                    1 => (SECOND, true),
                    2 => (THIRD, false),
                    index => panic!("unexpected OSC dispatch {index}"),
                };

                // Compare while the parser's borrowed OSC slices are valid.
                // The next dispatch reuses the same internal raw buffer.
                assert_eq!(params, expected_params);
                assert_eq!(bell_terminated, expected_bell);
                self.osc_index += 1;
            }
        }

        const INPUT: &[u8] = b"\x1b]99;i=one:p=body;alpha;beta\x1b\\\
            \x1b]9;second\x07\
            \x1b]777;notify;Third;gamma;delta\x1b\\";
        let mut dispatcher = ReferenceDispatcher::default();
        let mut parser = Parser::new();

        parser.advance(&mut dispatcher, INPUT);

        assert_eq!(dispatcher.osc_index, 3);
    }
