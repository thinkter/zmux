...
/// Reserved title prefix used to bridge notification OSC payloads through
/// terminal stacks which expose title changes but not unknown OSC commands.
///
/// Consumers must treat this as a transient protocol marker, never as a real
/// terminal title. `alacritty_terminal` wraps it with restoration metadata.
pub const ZMUX_OSC_NOTIFICATION_TITLE_PREFIX: &str = "\u{1f}zmux-osc-notification-v1:";

/// Keep a single title event from carrying an arbitrarily large notification.
/// Kitty permits 4096 encoded payload bytes, leaving ample room for metadata.
const MAX_ZMUX_OSC_NOTIFICATION_BYTES: usize = 8 * 1024;
...
fn bridged_notification_title(params: &[&[u8]]) -> Option<String> {
    let payload_len = params
        .iter()
        .try_fold(params.len().saturating_sub(1), |len, param| len.checked_add(param.len()))?;
    if payload_len > MAX_ZMUX_OSC_NOTIFICATION_BYTES {
        return None;
    }

    let mut title = String::with_capacity(ZMUX_OSC_NOTIFICATION_TITLE_PREFIX.len() + payload_len);
    title.push_str(ZMUX_OSC_NOTIFICATION_TITLE_PREFIX);
    for (index, param) in params.iter().enumerate() {
        if index > 0 {
            title.push(';');
        }
        title.push_str(str::from_utf8(param).ok()?);
    }
    Some(title)
}
    ...
    T: Timeout,
{
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            // Bridge notification protocols through a transient title marker.
            // Alacritty forwards the marker as a sequenced event without
            // storing it as title state.
            b"9" | b"99" | b"777" => {
                if let Some(title) = bridged_notification_title(params) {
                    self.handler.set_title(Some(title));
                } else {
                    unhandled(params);
                }
...
mod tests {
    ...
    #[test]
    fn notification_osc_payloads_are_bridged_without_reinterpretation() {
        let cases: &[&[u8]] = &[
            b"\x1b]9;build complete\x07",
            b"\x1b]99;i=build:p=body;tests; passed\x1b\\",
            b"\x1b]777;notify;Build;done\x07",
        ];
        ...
        assert_eq!(
            handler.titles,
            vec![
                Some(format!("{ZMUX_OSC_NOTIFICATION_TITLE_PREFIX}9;build complete")),
                Some(format!(
                    "{ZMUX_OSC_NOTIFICATION_TITLE_PREFIX}99;i=build:p=body;tests; passed"
                )),
                Some(format!("{ZMUX_OSC_NOTIFICATION_TITLE_PREFIX}777;notify;Build;done")),
            ]
        );
    }

    #[test]
    fn oversized_notification_osc_is_not_bridged() {
        let mut bytes = b"\x1b]9;".to_vec();
        bytes.extend(iter::repeat(b'x').take(MAX_ZMUX_OSC_NOTIFICATION_BYTES));
        bytes.push(0x07);
        ...
        assert!(handler.titles.is_empty());
    }

    #[test]
    fn notification_osc_preserves_more_than_sixteen_semicolon_fields() {
        let body = (0..32).map(|index| index.to_string()).collect::<Vec<_>>().join(";");
        let payload = format!("777;notify;Build;{body}");
        ...
        assert_eq!(
            handler.titles,
            vec![Some(format!("{ZMUX_OSC_NOTIFICATION_TITLE_PREFIX}{payload}"))]
        );
    }
