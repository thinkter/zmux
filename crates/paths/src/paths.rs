//! Paths to locations used by zmux.
//!
//! This is a deliberately small fork of the pinned Zed `paths` crate. Keeping
//! the same API lets the Zed crates embedded by zmux share this application's
//! namespace instead of touching a user's Zed data.
...
///
/// This must remain distinct from Zed to avoid colliding with user data.
pub const APP_NAME: &str = "Zmux";
...
/// On macOS, this is `~/Library/Application Support/Zmux`.
/// On Linux/FreeBSD, this is `$XDG_DATA_HOME/zmux`.
/// On Windows, this is `%LOCALAPPDATA%\Zmux`.
static CURRENT_DATA_DIR: OnceLock<PathBuf> = OnceLock::new();

/// The resolved config directory, combining custom override or platform defaults.
/// This is set once and cached for subsequent calls.
/// On macOS, this is `~/.config/zmux`.
/// On Linux/FreeBSD, this is `$XDG_CONFIG_HOME/zmux`.
/// On Windows, this is `%APPDATA%\Zmux`.
static CONFIG_DIR: OnceLock<PathBuf> = OnceLock::new();

/// Root used by `cargo test`. It is per-test-process by default, so test
/// startup never observes a developer's Zed or zmux state. Set
/// `ZMUX_TEST_ROOT` to make it deterministic for an integration harness.
#[cfg(feature = "test-support")]
pub fn test_root() -> &'static PathBuf {
    static TEST_ROOT: OnceLock<PathBuf> = OnceLock::new();
    TEST_ROOT.get_or_init(|| {
        let root = env::var_os("ZMUX_TEST_ROOT").map_or_else(
            || {
                let nonce = std::time::SystemTime::now()
                    .duration_since(std::time::UNIX_EPOCH)
                    .expect("system time must be after the Unix epoch")
                    .as_nanos();
                env::temp_dir().join(format!("zmux-test-{}-{nonce}", std::process::id()))
            },
            PathBuf::from,
        );
        std::fs::create_dir_all(&root).expect("failed to create zmux test root");
        root
    })
}

#[cfg(feature = "test-support")]
fn test_dir(name: &str) -> Option<PathBuf> {
    let path = test_root().join(name);
    std::fs::create_dir_all(&path).expect("failed to create isolated zmux test directory");
    Some(path)
}

#[cfg(not(feature = "test-support"))]
fn test_dir(_: &str) -> Option<PathBuf> {
    None
}

/// Returns the relative path to the zmux server directory on the SSH host.
pub fn remote_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zmux_server").unwrap());
    *CACHED
}

// Remove this once 223 goes stable
/// Returns the relative path to the zmux WSL server directory on the WSL host.
pub fn remote_wsl_server_dir_relative() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zmux_wsl_server").unwrap());
    *CACHED
}
...
/// Returns the path to the configuration directory used by zmux.
pub fn config_dir() -> &'static PathBuf {
    CONFIG_DIR.get_or_init(|| {
        if let Some(test_dir) = test_dir("config") {
            test_dir
...
/// Returns the path to the data directory used by zmux.
pub fn data_dir() -> &'static PathBuf {
    CURRENT_DATA_DIR.get_or_init(|| {
        if let Some(test_dir) = test_dir("data") {
            test_dir
        } else if let Some(custom_dir) = CUSTOM_DATA_DIR.get() {
            custom_dir.clone()
...
pub fn state_dir() -> &'static PathBuf {
    ...
    STATE_DIR.get_or_init(|| {
        if let Some(test_dir) = test_dir("state") {
            return test_dir;
        }
...
/// Returns the path to the cache directory used by zmux.
pub fn cache_dir() -> &'static PathBuf {
    static CACHE_DIR: OnceLock<PathBuf> = OnceLock::new();
    CACHE_DIR.get_or_init(|| {
        if let Some(test_dir) = test_dir("cache") {
            return test_dir;
        }
...
/// Returns the path to the temporary directory used by zmux.
///
/// Production keeps temporary files in zmux's cache directory. Test builds use
/// a separate `tmp` root so transient files cannot leak between test runs.
pub fn temp_dir() -> &'static PathBuf {
    static TEMP_DIR: OnceLock<PathBuf> = OnceLock::new();
    TEMP_DIR.get_or_init(|| test_dir("tmp").unwrap_or_else(|| cache_dir().clone()))
}
...
pub fn logs_dir() -> &'static PathBuf {
    ...
    LOGS_DIR.get_or_init(|| {
        if let Some(test_dir) = test_dir("logs") {
            test_dir
        } else if cfg!(target_os = "macos") {
...
/// Returns the path to the zmux server directory on this SSH host.
...
/// Returns the path to the `Zmux.log` file.
pub fn log_file() -> &'static PathBuf {
    static LOG_FILE: OnceLock<PathBuf> = OnceLock::new();
    LOG_FILE.get_or_init(|| logs_dir().join(format!("{}.log", APP_NAME)))
}

/// Returns the path to the `Zmux.log.old` file.
...
/// user opens, and is loaded into the native zmux agent's system prompt.
...
/// # Arguments
///
/// * `dev_mode` - If true, assumes the current working directory is the zmux repository.
...
///
/// This is where language servers are downloaded to for languages built into zmux.
...
///
/// This is where debug adapters are downloaded to for DAPs that are built into zmux.
...
/// Returns the relative path to a `.zmux` folder within a project.
...
pub fn local_settings_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zmux/settings.json").unwrap());
    *CACHED
}
...
pub fn local_tasks_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zmux/tasks.json").unwrap());
    *CACHED
}
...
pub fn local_debug_file_relative_path() -> &'static RelPath {
    static CACHED: LazyLock<&'static RelPath> =
        LazyLock::new(|| RelPath::unix(".zmux/debug.json").unwrap());
    *CACHED
}
...
pub fn global_gitignore_path() -> Option<PathBuf> {
    Some(config_dir().join("git").join("ignore"))
}
