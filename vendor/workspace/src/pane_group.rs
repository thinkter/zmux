...
impl PaneAxis {
    ...
    fn insert_pane(&mut self, idx: usize, new_pane: &Entity<Pane>) {
        ...
        *self.flexes.lock() = vec![1.; self.members.len()];
        // Keep the cached bounding boxes the same length as `members` so that
        // lookups between this mutation and the next layout pass stay in
        // bounds; the next paint recomputes the actual values.
    ...
    fn bounding_box_for_pane(&self, pane: &Entity<Pane>) -> Option<Bounds<Pixels>> {
        ...
        for (idx, member) in self.members.iter().enumerate() {
            match member {
                Member::Pane(found) => {
                    if pane == found {
                        // The cached boxes can briefly be shorter than
                        // `members` between a structural change and the next
                        // layout pass, so avoid indexing out of bounds.
    ...
    fn pane_at_pixel_position(&self, coordinate: Point<Pixels>) -> Option<&Entity<Pane>> {
        ...
        for (idx, member) in self.members.iter().enumerate() {
            if let Some(coordinates) = bounding_boxes.get(idx).copied().flatten()
