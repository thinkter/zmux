...
impl GitPanel {
    fn new(
        ...
        let scoped_repositories = crate::repository_scope(cx).repositories(&project, cx);
    ...
    fn update_visible_entries(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let scoped_repositories = crate::repository_scope(cx).repositories(&self.project, cx);
    ...
    pub fn render_footer(
        ...
        let git_panel = cx.entity();
        let display_name = crate::repository_scope(cx).display_name(&active_repository, cx);
    ...
    fn render_uninitialized_ui(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        if crate::repository_scope(cx)
            .repositories(&self.project, cx)
            .is_empty()
        {
            return v_flex()
                .gap_1()
                .items_center()
                .child(Label::new("No Git repository in this workspace").color(Color::Muted))
                .child(
                    Label::new("Open a terminal inside a repository to attach it.")
                        .size(LabelSize::Small)
                        .color(Color::Muted),
                )
                .into_any_element();
...
impl RenderOnce for PanelRepoFooter {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let single_repo = project
            .as_ref()
            .map(|project| crate::repository_scope(cx).repositories(project, cx).len() == 1)
            .unwrap_or(true);
