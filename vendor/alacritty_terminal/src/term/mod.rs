...
/// Bound the real title copied into zmux's transient notification envelope.
const ZMUX_NOTIFICATION_RESTORE_TITLE_MAX_BYTES: usize = 64 * 1024;
const ZMUX_NOTIFICATION_PAYLOAD_MAX_BYTES: usize = 8 * 1024;
/// Zed processes at most 100 terminal backend events in one entity update.
const ZMUX_NOTIFICATION_PENDING_MAX: usize = 128;
const ZMUX_NOTIFICATION_ENVELOPE_MAX_BYTES: usize = 1024 * 1024;
/// OSC 2 title prefix for authenticated, host-generated replay ACKs.
pub const ZMUX_OSC_NOTIFICATION_ACK_TITLE_PREFIX: &str = "zmux-osc-notification-v1:ack3;";
/// OSC 2 title prefix for authenticated, host-generated PTY responses.
pub const ZMUX_PTY_RESPONSE_TITLE_PREFIX: &str = "zmux-osc-notification-v1:pty3;";

const ZMUX_ACK_KEY_BYTES: usize = 32;
const ZMUX_ACK_TAG_BYTES: usize = 32;
const ZMUX_ACK_AUTH_DOMAIN: &[u8] = b"zmux-osc-notification-v1:trusted-ack3\0";
const ZMUX_PTY_RESPONSE_AUTH_DOMAIN: &[u8] = b"zmux-osc-notification-v1:trusted-pty3\0";
pub const ZMUX_PTY_RESPONSE_MAX_BYTES: usize = 8 * 1024;
const ZMUX_PTY_RESPONSE_ENCODED_MAX_BYTES: usize = (ZMUX_PTY_RESPONSE_MAX_BYTES + 2) / 3 * 4;
static ZMUX_ACK_KEY: LazyLock<Option<[u8; ZMUX_ACK_KEY_BYTES]>> =
    LazyLock::new(|| generate_zmux_control_key(|key| getrandom::fill(key).is_ok()));

type ZmuxAckHmac = Hmac<Sha256>;

/// Build an authenticated replay ACK for injection by trusted host code.
///
/// The HMAC key exists only inside this process and the sequence is intercepted
/// by [`Term`] before it can become an observable terminal title. Child PTY
/// output can reproduce the wire shape, but cannot forge the authentication
/// tag which authorizes replay deletion.
pub fn build_zmux_notification_replay_ack(watermark: u64) -> Option<Vec<u8>> {
    if watermark == 0 {
        return None;
    }

    let tag = zmux_notification_replay_ack_tag(watermark)?;
    let mut sequence = format!("\x1b]2;{ZMUX_OSC_NOTIFICATION_ACK_TITLE_PREFIX}{watermark};");
    for byte in tag {
        write!(sequence, "{byte:02x}").expect("writing a replay ACK tag to a String cannot fail");
    }
    sequence.push_str("\x1b\\");
    Some(sequence.into_bytes())
}

/// Build an authenticated PTY response for injection by trusted host code.
///
/// The response is bounded, UTF-8 validated, and Base64 encoded for transport
/// through an OSC title. [`Term`] verifies the HMAC and emits [`Event::PtyWrite`]
/// without treating the response as keyboard input.
pub fn build_zmux_pty_response(response: &[u8]) -> Option<Vec<u8>> {
    if response.len() > ZMUX_PTY_RESPONSE_MAX_BYTES || str::from_utf8(response).is_err() {
        return None;
    }

    let encoded = Base64.encode(response);
    debug_assert!(encoded.len() <= ZMUX_PTY_RESPONSE_ENCODED_MAX_BYTES);
    let tag = zmux_pty_response_tag(response)?;
    let mut sequence = format!(
        "\x1b]2;{ZMUX_PTY_RESPONSE_TITLE_PREFIX}{};{encoded};",
        response.len(),
    );
    for byte in tag {
        write!(sequence, "{byte:02x}").expect("writing a PTY response tag to a String cannot fail");
    }
    sequence.push_str("\x1b\\");
    Some(sequence.into_bytes())
}

fn generate_zmux_control_key(
    fill: impl FnOnce(&mut [u8; ZMUX_ACK_KEY_BYTES]) -> bool,
) -> Option<[u8; ZMUX_ACK_KEY_BYTES]> {
    let mut key = [0_u8; ZMUX_ACK_KEY_BYTES];
    fill(&mut key).then_some(key)
}

fn zmux_control_key() -> Option<&'static [u8; ZMUX_ACK_KEY_BYTES]> {
    ZMUX_ACK_KEY.as_ref()
}

fn zmux_notification_replay_ack_tag(watermark: u64) -> Option<[u8; ZMUX_ACK_TAG_BYTES]> {
    let mut mac = ZmuxAckHmac::new_from_slice(zmux_control_key()?)
        .expect("HMAC accepts a 32-byte replay ACK key");
    mac.update(ZMUX_ACK_AUTH_DOMAIN);
    mac.update(&watermark.to_be_bytes());
    Some(mac.finalize().into_bytes().into())
}

fn zmux_pty_response_tag(response: &[u8]) -> Option<[u8; ZMUX_ACK_TAG_BYTES]> {
    let mut mac = ZmuxAckHmac::new_from_slice(zmux_control_key()?)
        .expect("HMAC accepts a 32-byte control key");
    mac.update(ZMUX_PTY_RESPONSE_AUTH_DOMAIN);
    mac.update(&(response.len() as u64).to_be_bytes());
    mac.update(response);
    Some(mac.finalize().into_bytes().into())
}

fn authenticated_zmux_notification_replay_ack(title: &str) -> Option<u64> {
    let encoded = title.strip_prefix(ZMUX_OSC_NOTIFICATION_ACK_TITLE_PREFIX)?;
    let (watermark, encoded_tag) = encoded.split_once(';')?;
    let watermark = watermark.parse::<u64>().ok().filter(|value| *value != 0)?;
    let tag = decode_zmux_ack_tag(encoded_tag)?;
...
fn authenticated_zmux_pty_response(title: &str) -> Option<String> {
    let encoded = title.strip_prefix(ZMUX_PTY_RESPONSE_TITLE_PREFIX)?;
    let mut fields = encoded.split(';');
    let raw_len = fields.next()?.parse::<usize>().ok()?;
    let payload = fields.next()?;
    let encoded_tag = fields.next()?;
    if fields.next().is_some()
        || raw_len > ZMUX_PTY_RESPONSE_MAX_BYTES
        || payload.len() > ZMUX_PTY_RESPONSE_ENCODED_MAX_BYTES
    {
        return None;
    }

    let response = Base64.decode(payload).ok()?;
    if response.len() != raw_len {
        return None;
    }
    let tag = decode_zmux_ack_tag(encoded_tag)?;
    let mut mac = ZmuxAckHmac::new_from_slice(zmux_control_key()?)
...
fn decode_zmux_ack_tag(encoded: &str) -> Option<[u8; ZMUX_ACK_TAG_BYTES]> {
    if encoded.len() != ZMUX_ACK_TAG_BYTES * 2 || !encoded.is_ascii() {
        return None;
    }

    let mut tag = [0_u8; ZMUX_ACK_TAG_BYTES];
    for (output, pair) in tag.iter_mut().zip(encoded.as_bytes().chunks_exact(2)) {
...
#[derive(Debug)]
struct ZmuxPendingNotification {
    sequence: u64,
    payload: String,
}
...
pub struct Term<T> {
    ...
    /// Sequence used to distinguish replayed zmux bridge events.
    zmux_notification_sequence: u64,

    /// Notification payloads retained until the title consumer acknowledges
    /// the newest sequence it observed.
    zmux_pending_notifications: VecDeque<ZmuxPendingNotification>,
...
impl<T> Term<T> {
    ...
    pub fn new<D: Dimensions>(config: Config, dimensions: &D, event_proxy: T) -> Term<T> {
        ...
        Term {
            ...
            zmux_notification_sequence: Default::default(),
            zmux_pending_notifications: Default::default(),
        ...
        T: EventListener,
    {
        ...
        if self.zmux_pending_notifications.is_empty() {
            self.zmux_emit_current_title();
        } else {
            self.zmux_emit_replay_envelope();
        }
...
impl<T: EventListener> Term<T> {
    fn zmux_restore_metadata(&self) -> String {
        match self.title.as_deref() {
            Some(title) if title.len() <= ZMUX_NOTIFICATION_RESTORE_TITLE_MAX_BYTES => {
                format!("t{}", Base64.encode(title))
            }
            Some(_) | None => "n".to_owned(),
        }
    }

    fn zmux_replay_envelope_len(&self, restore: &str) -> usize {
        let Some(latest) = self.zmux_pending_notifications.back() else {
            return 0;
        };
        let count = self.zmux_pending_notifications.len();
        let header_len = ansi::ZMUX_OSC_NOTIFICATION_TITLE_PREFIX.len()
            + "@3;".len()
            + latest.sequence.to_string().len()
            + 1
            + restore.len()
            + 1
            + count.to_string().len()
        ...
        header_len
            + self
                .zmux_pending_notifications
                .iter()
                .map(|notification| {
                    notification.sequence.to_string().len()
                        + 1
                        + notification.payload.len().to_string().len()
                        + 1
    ...
    fn zmux_trim_pending_notifications(&mut self) {
        while self.zmux_pending_notifications.len() > ZMUX_NOTIFICATION_PENDING_MAX {
            self.zmux_pending_notifications.pop_front();
        }

        let restore = self.zmux_restore_metadata();
        while self.zmux_pending_notifications.len() > 1
            && self.zmux_replay_envelope_len(&restore) > ZMUX_NOTIFICATION_ENVELOPE_MAX_BYTES
        {
            self.zmux_pending_notifications.pop_front();
        }
    }

    fn zmux_replay_envelope(&self) -> Option<String> {
        let latest = self.zmux_pending_notifications.back()?;
        let restore = self.zmux_restore_metadata();
        let mut envelope = String::with_capacity(self.zmux_replay_envelope_len(&restore));
        write!(
            envelope,
            "{}@3;{};{};{};",
            ansi::ZMUX_OSC_NOTIFICATION_TITLE_PREFIX,
            latest.sequence,
            restore,
            self.zmux_pending_notifications.len(),
        )
        .expect("writing a zmux replay envelope to a String cannot fail");
        for notification in &self.zmux_pending_notifications {
            write!(
                envelope,
                "{},{}:{};",
            ...
            .expect("writing a zmux replay entry to a String cannot fail");
        }
        debug_assert!(envelope.len() <= ZMUX_NOTIFICATION_ENVELOPE_MAX_BYTES);
        Some(envelope)
    }

    fn zmux_emit_replay_envelope(&self) {
        if let Some(envelope) = self.zmux_replay_envelope() {
            self.event_proxy.send_event(Event::Title(envelope));
        }
    }

    fn zmux_emit_current_title(&self) {
        let event = match &self.title {
    ...
    /// Acknowledge a zmux notification replay through the trusted host API.
    ///
    /// This must never be reachable through [`Handler::set_title`] or another
    /// in-band PTY escape sequence: child processes control that byte stream.
    pub fn acknowledge_zmux_notification_replay(&mut self, sequence: u64) {
        let latest = self
            .zmux_pending_notifications
            .back()
            .map(|notification| notification.sequence);
        if sequence != 0 && Some(sequence) == latest {
            self.zmux_pending_notifications.clear();
            self.zmux_emit_current_title();
        } else {
            // A delayed ACK must not discard notifications which arrived
            // after it was emitted. Replay the current watermark instead.
            self.zmux_emit_replay_envelope();
        }
    }
}
...
impl<T: EventListener> Handler for Term<T> {
    ...
    fn set_title(&mut self, title: Option<String>) {
        if let Some(control) = title.as_deref() {
            if control.starts_with(ZMUX_OSC_NOTIFICATION_ACK_TITLE_PREFIX) {
                if let Some(sequence) = authenticated_zmux_notification_replay_ack(control) {
                    self.acknowledge_zmux_notification_replay(sequence);
                }
                return;
            }
            if control.starts_with(ZMUX_PTY_RESPONSE_TITLE_PREFIX) {
                if let Some(response) = authenticated_zmux_pty_response(control) {
        ...
        if let Some(transient_title) = title.as_deref().filter(|title| {
            title
                .strip_prefix(ansi::ZMUX_OSC_NOTIFICATION_TITLE_PREFIX)
                .is_some_and(|payload| payload.len() <= ZMUX_NOTIFICATION_PAYLOAD_MAX_BYTES)
        }) {
            // Zed exposes title events but not unknown OSC payloads, and it
            // collapses a backend batch into the final breadcrumb value. Keep
            // every unacknowledged payload in each title event so observing
            // only the final value cannot lose a notification.
            self.zmux_notification_sequence = self.zmux_notification_sequence.wrapping_add(1);
            if self.zmux_notification_sequence == 0 {
                self.zmux_notification_sequence = 1;
            }
            let payload = transient_title
                .strip_prefix(ansi::ZMUX_OSC_NOTIFICATION_TITLE_PREFIX)
                .expect("notification marker prefix was checked above")
                .to_owned();
            self.zmux_pending_notifications
                .push_back(ZmuxPendingNotification {
                    sequence: self.zmux_notification_sequence,
                    payload,
                });
            self.zmux_trim_pending_notifications();
            self.zmux_emit_replay_envelope();
            return;
        }
        ...
        if self.zmux_pending_notifications.is_empty() {
            ...
            // Preserve the current process title as restore state, but keep a
            // replay envelope as the observable title until it is acknowledged.
            self.zmux_trim_pending_notifications();
            self.zmux_emit_replay_envelope();
        }
    }
...
mod tests {
    ...
    #[test]
    fn notification_title_marker_emits_a_replay_envelope() {
        let size = TermSize::new(7, 17);
        let listener = RecordingListener::default();
        let events = listener.0.clone();
        let mut term = Term::new(Config::default(), &size, listener);
        term.set_title(Some("real title".into()));
        ...
        term.set_title(Some(marker));

        assert_eq!(term.title.as_deref(), Some("real title"));
        ...
        assert!(matches!(
            ...
            Event::Title(title)
                if title == &format!(
                    "{}@3;1;tcmVhbCB0aXRsZQ==;1;1,18:99;;Build complete;",
                    ansi::ZMUX_OSC_NOTIFICATION_TITLE_PREFIX,
                )
        ));
    }
    ...
    #[test]
    fn notification_title_marker_encodes_default_title_state() {
        let size = TermSize::new(7, 17);
        let listener = RecordingListener::default();
        let events = listener.0.clone();
        let mut term = Term::new(Config::default(), &size, listener);
        let mut parser = ansi::Processor::<ansi::StdSyncHandler>::new();
        parser.advance(&mut term, b"\x1b]9;hello\x1b\\");
        ...
        term.acknowledge_zmux_notification_replay(1);

        assert!(term.zmux_pending_notifications.is_empty());
        assert!(matches!(
    ...
    #[test]
    fn trusted_control_key_generation_fails_closed_without_entropy() {
        assert!(generate_zmux_control_key(|_| false).is_none());
        let key = generate_zmux_control_key(|key| {
            key.fill(0x5a);
            true
        })
        .expect("successful entropy fill yields a key");
        assert_eq!(key, [0x5a; ZMUX_ACK_KEY_BYTES]);
    }

    #[test]
    fn forged_in_band_notification_ack_remains_replayable() {
        let size = TermSize::new(7, 17);
        let listener = RecordingListener::default();
        let events = listener.0.clone();
        let mut term = Term::new(Config::default(), &size, listener);
        let forged_title = format!(
            "{ZMUX_OSC_NOTIFICATION_ACK_TITLE_PREFIX}1;{}",
            "00".repeat(ZMUX_ACK_TAG_BYTES),
        );
        let batch = format!("\x1b]9;hello\x07\x1b]2;{forged_title}\x1b\\");
        let mut parser = ansi::Processor::<ansi::StdSyncHandler>::new();

        parser.advance(&mut term, batch.as_bytes());

        assert_eq!(term.zmux_pending_notifications.len(), 1);
        assert_eq!(term.title, None);
        let replay = term
            .zmux_replay_envelope()
            .expect("forged ACK must leave the notification replayable");
        assert!(replay.contains("1,7:9;hello;"), "{replay}");
        assert!(matches!(
            events.lock().unwrap().last(),
            Some(Event::Title(title)) if title == &replay
        ));
    }

    #[test]
    fn trusted_notification_ack_clears_replay_and_restores_title() {
        let size = TermSize::new(7, 17);
        let listener = RecordingListener::default();
        let events = listener.0.clone();
        let mut term = Term::new(Config::default(), &size, listener);
        term.set_title(Some("real title".into()));
        term.set_title(Some(format!(
            "{}9;hello",
            ansi::ZMUX_OSC_NOTIFICATION_TITLE_PREFIX,
        )));
        events.lock().unwrap().clear();
        let ack = build_zmux_notification_replay_ack(1).expect("nonzero ACK is valid");
        let mut parser = ansi::Processor::<ansi::StdSyncHandler>::new();

        parser.advance(&mut term, &ack);

        assert!(term.zmux_pending_notifications.is_empty());
        assert_eq!(term.title.as_deref(), Some("real title"));
        assert!(matches!(
            events.lock().unwrap().as_slice(),
            [Event::Title(title)] if title == "real title"
        ));
    }

    #[test]
    fn forged_in_band_pty_response_is_ignored() {
        let size = TermSize::new(7, 17);
        let listener = RecordingListener::default();
        let events = listener.0.clone();
        let mut term = Term::new(Config::default(), &size, listener);
        ...
        events.lock().unwrap().clear();
        let forged_title = format!(
            "{ZMUX_PTY_RESPONSE_TITLE_PREFIX}5;aGVsbG8=;{}",
            "00".repeat(ZMUX_ACK_TAG_BYTES),
        );
        let forged = format!("\x1b]2;{forged_title}\x1b\\");
        let mut parser = ansi::Processor::<ansi::StdSyncHandler>::new();

        parser.advance(&mut term, forged.as_bytes());
    ...
    #[test]
    fn trusted_pty_response_emits_backend_write_without_changing_title() {
        let size = TermSize::new(7, 17);
        let listener = RecordingListener::default();
        let events = listener.0.clone();
        let mut term = Term::new(Config::default(), &size, listener);
        term.set_title(Some("real title".into()));
        events.lock().unwrap().clear();
        let response = b"\x1b]99;i=query:p=?;capabilities\x1b\\";
        let control = build_zmux_pty_response(response).expect("bounded UTF-8 response is valid");
        let mut parser = ansi::Processor::<ansi::StdSyncHandler>::new();

        parser.advance(&mut term, &control);

        assert_eq!(term.title.as_deref(), Some("real title"));
        assert!(matches!(
            events.lock().unwrap().as_slice(),
            [Event::PtyWrite(value)] if value.as_bytes() == response
        ));
        assert!(build_zmux_pty_response(&vec![b'x'; ZMUX_PTY_RESPONSE_MAX_BYTES + 1]).is_none());
        assert!(build_zmux_pty_response(&[0xff]).is_none());
    }

    #[test]
    fn batched_notifications_survive_a_genuine_title_until_acknowledged() {
        let size = TermSize::new(7, 17);
        let listener = RecordingListener::default();
        let events = listener.0.clone();
        let mut term = Term::new(Config::default(), &size, listener);
        term.set_title(Some("first real".into()));
        events.lock().unwrap().clear();
        ...
        parser.advance(
            &mut term,
            b"\x1b]99;;first\x1b\\\
              \x1b]2;second real\x1b\\\
              \x1b]777;notify;Build;done\x07",
        );

        assert_eq!(term.title.as_deref(), Some("second real"));
        let expected = [
            format!(
                "{}@3;1;tZmlyc3QgcmVhbA==;1;1,9:99;;first;",
                ansi::ZMUX_OSC_NOTIFICATION_TITLE_PREFIX,
            ),
            format!(
                "{}@3;1;tc2Vjb25kIHJlYWw=;1;1,9:99;;first;",
                ansi::ZMUX_OSC_NOTIFICATION_TITLE_PREFIX,
            ),
            format!(
                "{}@3;2;tc2Vjb25kIHJlYWw=;2;1,9:99;;first;2,21:777;notify;Build;done;",
                ansi::ZMUX_OSC_NOTIFICATION_TITLE_PREFIX,
            ),
        ];
        let recorded = events.lock().unwrap();
        assert_eq!(recorded.len(), expected.len());
        for (event, expected_title) in recorded.iter().zip(&expected) {
            match event {
                Event::Title(title) => assert_eq!(title, expected_title),
                event => panic!("expected title event, got {event:?}"),
            }
        }
        // Zed's BreadcrumbsChanged subscriber can observe only this final
        // value from the entire backend batch, so it must contain both OSCs.
        assert!(matches!(&recorded[2], Event::Title(title) if title == &expected[2]));
        drop(recorded);

        term.acknowledge_zmux_notification_replay(2);

        assert!(term.zmux_pending_notifications.is_empty());
        let events = events.lock().unwrap();
        assert!(matches!(events.last(), Some(Event::Title(title)) if title == "second real"));
    }

    #[test]
    fn replay_envelope_carries_a_full_zed_sized_burst() {
        let size = TermSize::new(7, 17);
        let listener = RecordingListener::default();
        let events = listener.0.clone();
        let mut term = Term::new(Config::default(), &size, listener);
        ...
        for index in 0..101 {
            input.extend_from_slice(format!("\x1b]9;n{index}\x07").as_bytes());
        }
        input.extend_from_slice(b"\x1b]2;after burst\x1b\\");

        parser.advance(&mut term, &input);

        assert_eq!(term.zmux_pending_notifications.len(), 101);
        assert_eq!(term.title.as_deref(), Some("after burst"));
        let mut expected = format!(
            "{}@3;101;tYWZ0ZXIgYnVyc3Q=;101;",
            ansi::ZMUX_OSC_NOTIFICATION_TITLE_PREFIX,
        );
        for index in 0..101 {
            let payload = format!("9;n{index}");
        ...
        assert!(matches!(events.last(), Some(Event::Title(title)) if title == &expected));
    }

    #[test]
    fn stale_trusted_notification_ack_replays_the_latest_envelope() {
        let size = TermSize::new(7, 17);
        let listener = RecordingListener::default();
        let events = listener.0.clone();
        let mut term = Term::new(Config::default(), &size, listener);
        ...
        events.lock().unwrap().clear();
        let mut parser = ansi::Processor::<ansi::StdSyncHandler>::new();
        parser.advance(&mut term, b"\x1b]9;one\x07\x1b]9;two\x07");

        let latest = match events.lock().unwrap().last() {
            Some(Event::Title(title)) => title.clone(),
            event => panic!("expected replay title event, got {event:?}"),
        };
        events.lock().unwrap().clear();
        term.acknowledge_zmux_notification_replay(1);

        assert_eq!(term.zmux_pending_notifications.len(), 2);
        assert!(matches!(
            events.lock().unwrap().as_slice(),
            [Event::Title(title)] if title == &latest
        ));
        events.lock().unwrap().clear();
    ...
    #[test]
    fn replay_queue_drops_only_the_oldest_entry_above_its_cap() {
        let size = TermSize::new(7, 17);
        let listener = RecordingListener::default();
        let events = listener.0.clone();
        let mut term = Term::new(Config::default(), &size, listener);

        for index in 0..=ZMUX_NOTIFICATION_PENDING_MAX {
            let marker = format!("{}9;n{index}", ansi::ZMUX_OSC_NOTIFICATION_TITLE_PREFIX,);
            term.set_title(Some(marker));
            events.lock().unwrap().clear();
        }

        assert_eq!(
            term.zmux_pending_notifications.len(),
            ZMUX_NOTIFICATION_PENDING_MAX
        );
        assert_eq!(
            term.zmux_pending_notifications
                .front()
                .map(|notification| notification.sequence),
            Some(2),
        );
        assert_eq!(
            term.zmux_pending_notifications
                .back()
                .map(|notification| notification.sequence),
            Some((ZMUX_NOTIFICATION_PENDING_MAX + 1) as u64),
        );
    }
