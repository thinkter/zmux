...
/// Host-level policy for repository discovery and selection. Zed's default
/// implementation exposes every repository known to the Project; embedders
/// such as zmux can scope that list to their own notion of a workspace.
pub trait RepositoryScope: Send + Sync + 'static {
...
struct DefaultRepositoryScope;

impl RepositoryScope for DefaultRepositoryScope {
    fn repositories(
        &self,
...
struct RepositoryScopeGlobal(Arc<dyn RepositoryScope>);
impl gpui::Global for RepositoryScopeGlobal {}

pub fn set_repository_scope(scope: Arc<dyn RepositoryScope>, cx: &mut App) {
    cx.set_global(RepositoryScopeGlobal(scope));
}

pub(crate) fn repository_scope(cx: &App) -> Arc<dyn RepositoryScope> {
    cx.try_global::<RepositoryScopeGlobal>()
        .map(|scope| scope.0.clone())
        .unwrap_or_else(|| Arc::new(DefaultRepositoryScope))
}
